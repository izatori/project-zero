...
public class UserDomainService : IUserDomainService
{
    ...
    public UserDomainService(IUserRepository userRepository, IEmailDomainService emailDomainService)
    {
        _userRepository = userRepository ?? throw new ArgumentNullException(nameof(userRepository));
        _emailDomainService = emailDomainService ?? throw new ArgumentNullException(nameof(emailDomainService));
    }
    ...
    public async Task<bool> EmailIsUniqueAsync(string email, CancellationToken cancellationToken = default)
    {
        ...
        if (!_emailDomainService.IsValidEmail(email))
            return false;
