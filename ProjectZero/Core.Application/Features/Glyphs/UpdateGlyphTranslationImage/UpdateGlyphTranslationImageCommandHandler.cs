...
public class UpdateGlyphTranslationImageCommandHandler : IRequestHandler<UpdateGlyphTranslationImageCommand>
{
    private readonly IGlyphRepository _glyphRepository;
    private readonly IUnitOfWork _unitOfWork;

    public UpdateGlyphTranslationImageCommandHandler(IGlyphRepository glyphRepository, IUnitOfWork unitOfWork)
    {
        _glyphRepository = glyphRepository ?? throw new ArgumentNullException(nameof(glyphRepository));
        _unitOfWork = unitOfWork ?? throw new ArgumentNullException(nameof(unitOfWork));
    }

    public async Task Handle(UpdateGlyphTranslationImageCommand command, CancellationToken cancellationToken = default)
    {
